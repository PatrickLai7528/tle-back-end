import { IImportedRepository, IRecentRepo, ICommit } from './../entity/types';
...
export default class RepositoryService extends Service {
   ...
   public async findCommitBySha(ownerId: string, repoName: string, commitSha: string): Promise<ICommit | undefined> {
      const repo: IImportedRepository = (await this.find({ ownerId: ownerId, name: repoName }))[0];
   ...
   public async find(t: Partial<IImportedRepository>): Promise<IImportedRepository[]> {
      return await this.getCRUD().read(t, this.getModel()) as IImportedRepository[];
   }
   ...
   public async findRepoSortedUpdate(ownerId: string): Promise<IRecentRepo[]> {
      const repos: IImportedRepository[] = await this.getModel().find({ ownerId }).sort("lastUpdateAt");
   ...
   public async findByOwnerId(ownerId: string): Promise<IImportedRepository[]> {
      const repos: IImportedRepository[] = await this.getModel().find({ ownerId }) as IImportedRepository[];
      return repos;
   }
}